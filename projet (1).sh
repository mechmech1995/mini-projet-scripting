...
while true; do
ask=`zenity --list --title="choisir les options" --width=700 --height=500 --column="choisir une option" "1-lancer la commande top" "2-lancer la commande ps-aux" "3-afficher les processus lances par l'utilisateur root" "4-pour afficher les processus les plus gourmandes en terme cpu" "5-demande de help" "6-afficher les courbes des processus les plus gourmands en terme de cpu"   "Quitter"  --hide-header`
...
ps -eo pmem,pcpu,pid,args | tail -n +2 | sort -rnk 1 | head -5 >repert
zenity --list --width=700 --height=500 --column=commande <repert
fi
...
if [ "$ask" == "6-afficher les courbes des processus les plus gourmands en terme de cpu" ]; then
help >repert
zenity --list --width=700 --height=500 --column=commande <repert
fi
...
if [ "$1" == "-mcpu" ]; then
    ps -eo pmem,pcpu,pid,args | tail -n +2 | sort -rnk 1 | head -5 
fi
if [ "$1" == "-help" ]; then
    help
fi
